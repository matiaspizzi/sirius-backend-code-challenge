...
import { getUserById, updateUserQuota } from '../controllers/user.controller';
import { newMail } from '../controllers/mail.controller';
...
router.post('/send', async (req, res) => {
    ...
    if (user.quota >= 1000)
        return res
            .status(403)
            .json({ error: 'Limite de cuota diaria alcanzado' });
